...
<div>
    ...
    @if ($InvoiceSaved)
        <div class="alert alert-info">{{ trans('Dashboard/single_invoices.data_saved') }}</div>
    @endif

    @if ($InvoiceUpdated)
        <div class="alert alert-info">{{ trans('Dashboard/single_invoices.data_edit') }}</div>
    @endif
    ...
    @else
        <form wire:submit.prevent="store" autocomplete="off">
            ...
            <div class="row">
                <div class="col">
                    <label> {{ trans('Dashboard/single_invoices.Patient_name') }}</label>
                    <select wire:model="patient_id" class="form-control" required>
                        <option value="">-- {{ trans('Dashboard/single_invoices.Choose_list') }} --</option>
                ...
                <div class="col">
                    <label>{{ trans('Dashboard/single_invoices.doctor_name') }}</label>
                ...
                <div class="col">
                    <label> {{ trans('Dashboard/single_invoices.Invoice_type') }}</label>
                    <select wire:model="type" class="form-control" {{ $updateMode == true ? 'disabled' : '' }}>
                        <option value="">-- {{ trans('Dashboard/single_invoices.Choose_list') }} --</option>
                        <option value="1"> {{ trans('Dashboard/single_invoices.monetary') }}</option>
                        <option value="2"> {{ trans('Dashboard/single_invoices.Late_payment') }}</option>
                    </select>
                </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <select wire:model="Service_id" class="form-control"
                                                    ...
                                                    <option value="">--
                                                        {{ trans('Dashboard/single_invoices.Choose_a_service') }} --
                                                    </option>
                                                    @foreach ($Services as $Service)
                                                        <option value="{{ $Service->id }}">{{ $Service->nameLang }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </td>
            ...
            <input class="btn btn-outline-success" type="submit"
                value=" {{ trans('Dashboard/single_invoices.Confirm_data') }}">
        </form>
    @endif
</div>
